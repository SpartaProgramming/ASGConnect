...
void DisplayManager::updateGameUI(const GameState &state) {
  ...
  if (state.phase != PHASE_SETUP) { 
    if (state.team == TEAM_BLUE) tft.setTextColor(ST77XX_BLUE);
  ...
  if (state.phase == PHASE_SETUP) {
    ...
    if (state.endMessage.indexOf("BLUE") >= 0) tft.setTextColor(ST77XX_BLUE);
    else if (state.endMessage.indexOf("RED") >= 0) tft.setTextColor(ST77XX_RED);
  ...
  if (state.phase == PHASE_RUNNING || state.phase == PHASE_READY) {
    tft.fillRect(0, 150, 320, 20, ST77XX_BLACK); // Czyszczenie tła dla samego GPS
    tft.setTextSize(1);
